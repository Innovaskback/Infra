...
VIDEOS_DIR="$HLS_ROOT/videos"
CACHE_DIR="/var/cache/nginx/hls"
LOG_DIR="/var/log/nginx/hls"


echo -e "${BLUE}=========================================${NC}"
echo -e "${BLUE}    HLS Server Setup for Education     ${NC}"
echo -e "${BLUE}=========================================${NC}"
...
sudo apt install -y nginx htop iotop sysstat curl wget vim ufw

# Verify nginx installation
if command_exists nginx; then
    print_status "Nginx installed successfully: $(nginx -v 2>&1)"
else
...
print_status "Step 2: Creating directory structure..."
sudo mkdir -p $HLS_ROOT/videos
...
print_status "Step 3: Setting up permissions..."
...
sudo chown -R $current_user:www-data $VIDEOS_DIR
sudo chmod -R 755 $VIDEOS_DIR

print_status "Permissions configured successfully"

# 4. Apply System Optimizations
print_status "Step 4: Applying system optimizations..."
sudo tee /etc/sysctl.d/99-hls.conf > /dev/null << 'EOF'
# Network optimizations for HLS streaming
...
sudo sysctl -p /etc/sysctl.d/99-hls.conf

# Verify key settings
print_status "Verifying system optimizations..."
...
print_status "Step 5: Updating system limits..."
...
# 6. Update systemd settings
print_status "Step 6: Updating systemd settings..."
...
# Optimized for Educational Platform
...
http {
    ...
    gzip_types text/plain text/css application/json text/html;
...
server {
    ...
    location ~ /\.(env|cfg|py|js|json|config|git)$ {
        deny all;
        access_log off;
        return 404;
    }
    
    location ~ /(admin|Admin|config|settings|staging|api|v1|\.git|Crm|st/test|en/test) {
        deny all;
        access_log off;
        return 404;
    }
...
# 9. Enable the new site configuration
print_status "Step 9: Enabling HLS server configuration..."
sudo rm -f /etc/nginx/sites-enabled/default
sudo rm -f /etc/nginx/sites-enabled/hls-optimized.conf
sudo ln -sf /etc/nginx/sites-available/hls.conf /etc/nginx/sites-enabled/

# 10. Create Management Script
print_status "Step 10: Creating management script..."
...
case "$1" in
    start)
        ...
        echo "HLS Server started successfully."
        ;;
    
    stop)
        echo "Stopping HLS Server..."
        systemctl stop nginx
        echo "HLS Server stopped."
        ;;
    
    restart)
        echo "Restarting HLS Server..."
        systemctl restart nginx
        echo "HLS Server restarted successfully."
        ;;
    ...
    status)
        echo "=== HLS Server Status ==="
        systemctl status nginx --no-pager -l | head -15
        echo ""
        echo "=== Current Connections ==="
        curl -s http://127.0.0.1/nginx_status 2>/dev/null || echo "Status endpoint not available"
        echo ""
        echo "=== System Resources ==="
        echo "CPU: $(top -bn1 | grep "Cpu(s)" | awk '{print $2}' | cut -d'%' -f1)%"
        echo "Memory: $(free | grep Mem | awk '{printf "%.1f%%", $3/$2 * 100.0}')"
        echo "Disk: $(df /var/www/hls | tail -1 | awk '{print $5}')"
        ;;
    ...
    logs)
        echo "Showing recent access logs..."
